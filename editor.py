...
class Editor:
    def listen_events(self, mpos):
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                self.mouse_down_event(event, mpos)
    ...
    def mouse_down_event(self, event, mpos):
        if event.button == 1:
            self.clicking = True
            if not self.ongrid:
    ...
    def handle_tile_placement(self, tile_pos, current_tile_img, mpos):
        if self.ongrid:
    ...
    def run(self):
        while True:
            ...
            self.handle_tile_placement(tile_pos, current_tile_img, mpos)
            self.display.blit(current_tile_img, (5 , 5))

            self.listen_events(mpos)
